...
export const ABTestingPanel = ({ history }) => {
  ...
  const avgLatencyA =
    modelA === 0
      ? 0
      : Math.round(
          history.filter((item) => item.model_used === "A").reduce((sum, item) => sum + item.latency_ms, 0) / modelA
        );
  const avgLatencyB =
    modelB === 0
      ? 0
      : Math.round(
          history.filter((item) => item.model_used === "B").reduce((sum, item) => sum + item.latency_ms, 0) / modelB
        );
